...
export const ModalNovaCautela: React.FC<ModalNovaCautelaProps> = ({ modulo, onClose, onSuccess }) => {
  ...
  const handleAddLote = (id_lote: number) => {
    ...
    const defaultQty = Math.min(lote?.quantidade_atual || 15, lote?.calibre === '5.56x45mm' ? 120 : 45);
          ...
          <div>
            ...
            <div className="space-y-2">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Buscar recebedor por Matrícula (ex: PM-240992-1), Nome ou Graduação..."
                  value={searchPolicial}
                  onChange={(e) => setSearchPolicial(e.target.value)}
                  className="w-full pl-9 pr-3 py-2 bg-white border border-slate-300 rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-600 text-xs shadow-xs"
                />
              </div>
              ...
              <div className="max-h-32 overflow-y-auto rounded-lg border border-slate-200 bg-white divide-y divide-slate-100">
                {filteredPoliciais.length === 0 ? (
                  <div className="p-3 text-center text-slate-400 text-xs">Nenhum policial encontrado.</div>
                ) : (
                  filteredPoliciais.map((p, pIdx) => {
                    const isSelected = Number(selectedPolicialId) === p.id_policial;
                    return (
                      <button
                        key={`${p.id_policial || 'pol'}-${p.matricula || ''}-${pIdx}`}
                        type="button"
                        onClick={() => setSelectedPolicialId(p.id_policial)}
                      ...
                      >
                        <div className="flex items-center space-x-2">
                          <span className="font-bold text-slate-900">{p.patente} {p.nome_guerra}</span>
                          <span className="text-[11px] text-slate-500 font-normal">({p.nome_completo})</span>
                        </div>
              ...
              {selectedPolicial && (
                <div className="p-2.5 bg-blue-50 border border-blue-200 rounded-lg text-blue-900 flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span>
                      Selecionado: <strong>{selectedPolicial.patente} {selectedPolicial.nome_guerra}</strong> ({selectedPolicial.matricula})
                    </span>
                  </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="font-bold text-slate-900">
                3. Equipamentos Disponíveis na Reserva ({availableItems.length} itens)
              </label>
              <span className="text-[11px] text-slate-500 font-medium">
                Busca por Tombo, Série ou Nome
              </span>
            </div>
            ...
            <div className="space-y-2">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Buscar equipamento por tombo, número de série ou nome (ex: TS9, APX, TB-6BPM...)"
              ...
              <div className="max-h-36 overflow-y-auto rounded-lg border border-slate-200 bg-white divide-y divide-slate-100">
                {filteredEquipments.length === 0 ? (
                  <div className="p-3 text-center text-slate-400 text-xs">
                    {searchEquipment ? 'Nenhum equipamento encontrado com esses termos.' : 'Todos os itens disponíveis listados abaixo ou já selecionados.'}
                  </div>
                ) : (
                    ...
                    >
                      <div className="space-y-0.5">
                        ...
                        <div className="text-[10px] text-slate-500 font-mono flex items-center space-x-3">
                          <span>Tombo: {it.numero_tombo || '-'}</span>
                          <span>Série: {it.numero_serie || '-'}</span>
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 rounded bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs inline-flex items-center space-x-1 shadow-xs transition"
                      >
            ...
            {selectedItems.length > 0 && (
              <div className="mt-3 space-y-2 border border-slate-200 rounded-xl p-3 bg-slate-50">
            ...
            <div>
              ...
              >
                <option value="" disabled>
                  + Adicionar munição por calibre/especificação...
                </option>
                {availableLotes
