...
import { getActiveSupabaseConfig, testSupabaseConnection } from '../../services/supabaseClient';
...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess }) => {
  ...
  const operadores = db.getOperadores();
  ...
  const handleSelectQuickAccount = (email: string, initialPass: string = '123') => {
    setIdentificador(email);
    setSenha(initialPass);
    setErrorMessage(null);
  };
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-2xl shadow-black/60 backdrop-blur-xl relative">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-400">Padrão inicial: 123</span>
                </div>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
              ...
              <div className="pt-2">
                <button
                  ...
                  type="submit"
                  disabled={isLoading}
                  className="w-full py-3 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 active:bg-blue-700 disabled:opacity-50 text-white font-bold text-sm shadow-lg shadow-blue-600/30 transition flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
                >
                  {isLoading ? (
                    <>
            ...
            <div className="mt-5 pt-4 border-t border-slate-800 text-center">
              ...
              >
                ...
                <span>{showDemoAccounts ? 'Ocultar Perfis de Demonstração' : 'Ver Perfis de Demonstração (1-Clique)'}</span>
              </button>

              {/* Demo Accounts List */}
              {showDemoAccounts && (
                <div className="mt-3 space-y-1.5 text-left bg-slate-950/70 p-3 rounded-xl border border-slate-800 animate-in fade-in">
                  ...
                  {operadores.map(({ operador, policial }) => (
                    <button
                      ...
                      type="button"
                      onClick={() => handleSelectQuickAccount(operador.email || '', operador.senha || '123')}
                      className="w-full px-2.5 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-800 text-left flex items-center justify-between text-xs transition"
                    >
                      <div className="flex items-center gap-2 truncate">
                        <div className="p-1 rounded bg-slate-800 text-blue-400">
                          {operador.perfil_acesso === 'Superuser' && <Shield className="w-3 h-3" />}
                          {operador.perfil_acesso === 'P4' && <UserCheck className="w-3 h-3" />}
                          {operador.perfil_acesso === 'Armeiro' && <Crosshair className="w-3 h-3" />}
                          {operador.perfil_acesso === 'Rádio' && <Radio className="w-3 h-3" />}
                        </div>
                      ...
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-blue-950 text-blue-300 border border-blue-800 font-bold ml-2">
