    ...
    <div className="space-y-4">
      ...
      {allAlocacoes.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-sm font-bold text-slate-900 flex items-center space-x-2">
            <Building2 className="w-4 h-4 text-indigo-600" />
            <span>Alocações de Informática por Setor / Companhia</span>
          </h2>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <h2 className="text-sm font-bold text-slate-900">Inventário Geral de Informática</h2>
          <div className="relative w-full sm:w-72">
            <Search className="w-4 h-4 absolute left-3 top-2.5 text-slate-400" />
            <input
              type="text"
              placeholder="Buscar por tombo, série ou modelo..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-9 pr-3 py-2 bg-white border border-slate-300 rounded-lg text-slate-900 placeholder-slate-400 text-xs focus:outline-none focus:border-cyan-600 shadow-xs"
            />
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-700 font-bold border-b border-slate-200">
              <tr>
                ...
                <th className="p-3">Item / Fabricante</th>
                <th className="p-3">Configuração de Hardware</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredItens.map((it, idx) => (
                <tr key={`${it.id_item || 'info'}-${idx}`} className="hover:bg-slate-50 transition">
                  ...
                  <td className="p-3 text-slate-700">
                    {it.detalhe_informatica?.configuracao_resumida || it.observacao || '-'}
                  </td>
                  <td className="p-3">
                    <span
