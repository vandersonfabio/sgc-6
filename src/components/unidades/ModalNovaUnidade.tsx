...
import { TipoUnidade } from '../../types/database';
import {
  Building2,
  X,
  Plus,
...
export const ModalNovaUnidade: React.FC<ModalNovaUnidadeProps> = ({ onClose, onSuccess }) => {
  const { db, unidades, isSuperuser } = useDatabase();

  const [nome, setNome] = useState('');
  const [sigla, setSigla] = useState('');
  const [tipoUnidade, setTipoUnidade] = useState<TipoUnidade>('DPM');
  const [municipio, setMunicipio] = useState('Caicó');
  const [idUnidadeSuperior, setIdUnidadeSuperior] = useState<number | ''>(1);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
        <div className="bg-white rounded-2xl p-6 max-w-md w-full text-center space-y-4">
          ...
          <p className="text-xs text-slate-600">
            Apenas operadores com perfil de <strong>Superuser</strong> têm permissão para cadastrar novas unidades e setores.
          </p>
          <button
            onClick={onClose}
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (res.success) {
      setSuccessMsg(`Unidade "${nome}" cadastrada com sucesso!`);
      setTimeout(() => {
        onSuccess();
      }, 1200);
    } else {
      setError(res.error || 'Erro ao cadastrar unidade.');
      ...
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h2 className="text-base font-bold text-white tracking-tight">Cadastrar Unidade / Setor</h2>
                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-400 text-slate-950 uppercase">
                  Superuser
                </span>
              </div>
              <p className="text-xs text-blue-200">
                Estrutura organizacional, companhias, destacamentos e seções • 6º BPM
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">
                Comandante / Chefe da Seção (Opcional)
              </label>
              <div className="relative">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">
                Telefone / Rádio Frequência (Opcional)
              </label>
              <div className="relative">
